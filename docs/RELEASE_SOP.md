...
| 2026-08-25 | v0.1.8 | `gh release create ... --latest=false` | GitHub kept v0.1.7 as the Latest release after v0.1.8 was published | The manual fallback command explicitly disabled the Latest marker even though this was the stable replacement release | Ran `gh release edit v0.1.8 --latest` and verified the release list now selects v0.1.8 | Stable manual releases must mark the new patch as Latest unless the requester explicitly asks for a non-latest fallback build |
| 2026-08-25 | v0.1.8 | `gh release view v0.1.8 --json ...,isLatest,...` | `Unknown JSON field: "isLatest"` | This gh version exposes the Latest marker on `gh release list`, not `gh release view` | Reran `release view` with supported fields and checked `release list --json tagName,isLatest` separately | Keep command-specific gh JSON field sets and use `release list` for Latest status |
| 2026-08-25 | v0.1.9 | Cookie import pruning after the v0.1.8 fix | The low-value-cookie cleanup path could also delete the four app-managed rejection cookies, allowing a consent prompt to return after a large import | Preserve only managed ChatGPT rejection cookies during pruning, add an origin-boundary test, and publish the corrective patch as v0.1.9 without moving v0.1.8 | Treat app-managed consent state as protected product state even when pruning non-essential cookies; test import cleanup and consent persistence together |
