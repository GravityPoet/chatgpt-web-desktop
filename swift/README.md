...
- 本机输入草稿静默保存：按账号空间和会话保留最近输入，正常聊天不添加提示或改变输入框布局；收到回复、输入框变空或关闭保存开关都不会清除本机副本。在“编辑 → 恢复本页保存的草稿”主动恢复，不覆盖已有输入；仅主动清除时删除，支持撤销
...
- 下载中心：常规下载及图像下载统一显示数量、进度、完成/失败状态；保留最近 20 项，支持在 Finder 中显示、复制路径及可恢复任务重试。默认不打开 Finder，可在设置中开启。无痕记录不落盘；源页面关闭后需回原页面重新下载
- 网络恢复：区分断网、域名解析、连接超时、HTTP 服务错误、Cloudflare 验证和 WebKit 进程恢复；联网后仅对失败的页面重试一次，不重发问题
- 工具栏账号空间切换器可自定义显示/隐藏；切换时保存各空间窗口和聊天页面位置，页面结构未改变时恢复滚动位置，用户开始操作后停止恢复
- 可选快速窗口：默认关闭，可录制全局快捷键并报告占用冲突；使用当前账号空间的小窗口直接聊天和插入备忘录。关闭窗口后释放 WebView
- 输入与回复流畅度：草稿读取避免强制布局；完成状态只跟踪相关控件，流式文字更新不再触发整页扫描；图片保存放到后台队列，下载进度合并更新
